...
def run_python_script(package=None, module=None, args=[], p_args=[]):
    ...
    assert isinstance(args,
                      (tuple, list)) and isinstance(p_args, (tuple, list))
...
def run_python_script_in_terminal(
    ...
    if os.name == 'nt':
        ...
        try:
            ...
            QMessageBox.critical(
                ...
                _('Run'),
                _(
                    "It was not possible to run this file in "
...
def is_module_installed(
    module_name, version=None, installed_version=None, interpreter=None
):
    """
    ...
    if interpreter:
        ...
        if actver is None and version is not None:
            ...
            if not symb:
                symb = '='
            msg = "Invalid version condition '%s'" % symb
            assert symb in ('>=', '>', '=', '<', '<='), msg
...
def is_python_interpreter(filename):
    ...
    if (not osp.isfile(real_filename) or
            encoding.is_text_file(real_filename) or
...
def local_test_programs():
    """Run local test for programs module."""
